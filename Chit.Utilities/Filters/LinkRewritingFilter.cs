...
public class LinkRewritingFilter : IAsyncResultFilter
    {
        ...
        private static void RewriteAllLinks(object model, LinkRewriter rewriter)
        {
            ...
            foreach (var linkProperty in linkProperties)
            {
                ...
                if (linkProperty.Name == nameof(ChitStandardResponse<object>.Self))
                {
                    allProperties
                        .SingleOrDefault(p => p.Name == nameof(ChitStandardResponse<object>.Href))
                        ?.SetValue(model, rewritten.Href);

                    allProperties
                        .SingleOrDefault(p => p.Name == nameof(ChitStandardResponse<object>.Method))
                        ?.SetValue(model, rewritten.Method);

                    allProperties
                        .SingleOrDefault(p => p.Name == nameof(ChitStandardResponse<object>.Relations))
